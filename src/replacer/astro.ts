...
class AstroTemplateParser {
  ...
  createAst() {
    ...
    const pureJSX = htmlCommentToJsx(this.code)
      ...
      .replace(/(<style[\s\S]*?<\/style>)/g, (match, p1) => {
        ...
        return `<S${key}></S${key}>`
      }).replace(/<script([^>]*?)>([\s\S]*?)<\/script>/g, (match, p1, p2) => {
  ...
  replacerJSinJSX(replacer: (ast: GoGoAST) => GoGoAST) {
    this.jsReplacer = Object.fromEntries(Object.entries(this.jsReplacer).map(([key, value]) => {
  ...
  createReplacer(config: ReplacerConfig) {
    ...
    return (ast: AstroAST) => {
      ...
      return {
        ...
        generate() {
          return `---\n${js.generate()}\n---\n${p.replaceSlotToCode(jsx.generate().slice(3, -3))}`;
        },
      };
    };
  },
